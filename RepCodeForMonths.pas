...
type
  TRepCodetForMonthsForm = class(TForm)
    TopPn: TPanel;
    CaptionLB: TLabel;
    MainPn: TPanel;
    SB: TScrollBox;
    ...
    procedure PrepareCellData(id : string; dt1,dt2 : TDate);
    procedure FormResize(Sender: TObject);
    procedure CheckAllBtnClick(Sender: TObject);
    function  GetProdSQL(dt1,dt2 : TDateTime):string;
  private
...
function  TRepCodetForMonthsForm.GetCenterFilterString : string;
var
  pid     : ^integer;
begin
  //отбор по региону
...
function  TRepCodetForMonthsForm.GetReportFilterString(dt1,dt2 : TDate) : string;
var
  flt,str : string;
  i       : integer;
  pid     : ^integer;
...
function  TRepCodetForMonthsForm.GetRecordFilterString(dt1,dt2 : TDate) : string;
var
  flt,str : string;
  i       : integer;
  pid     : ^integer;
...
begin
  ...
  //по группам
  str:='';
  if (GroupCB.Visible)and(GroupCheck.Checked)and(GroupCheck.Enabled) then begin
    str:='';
...
procedure TRepCodetForMonthsForm.PrepareCellData(id : string; dt1,dt2 : TDate);
var
...
begin
  ...
  filter:=' AND (WORKCODE='+QuotedStr(id)+')';
  ...
  if (self.ValCB.ItemIndex>0) then
    sql:=stringreplace(SQL,'/PRODFILTER/',self.GetProdSQL(dt1,dt2),[rfReplaceAll])
...
function  TRepCodetForMonthsForm.GetWorkCodeCheckList: string;
var
  i    : integer;
  pstr : ^string;
  str  : string;
...
function  TRepCodetForMonthsForm.GetCheckedFromList(list : TCheckListBox):string;
var
  i    : integer;
  pint : ^integer;
begin
...
procedure TRepCodetForMonthsForm.PrepareBtnClick(Sender: TObject);
var
  i,c,r,prod : integer;
  RowQuery   : TADOQuery;
  mainfilter,
...
begin
  ...
  sql.Add('SELECT T1.CODE AS ID, T1.MAINTYPE AS MT, CONCAT(T2.DESCR,'+QuotedStr(' ')+',T1.CODE,'+QuotedStr(' ')+',T1.DESCR) AS DESCR ,MAINTYPE');
  sql.Add('FROM [SERVCODES] AS T1');
  sql.Add('LEFT JOIN [MAINTYPES] AS T2 ON T2.[ID]=T1.[MAINTYPE]');
  sql.Add('WHERE (LEN(T1.DESCR)>0) AND (ISFOLDER=0)');
  mainfilter:='';
  if self.MainTypeCheck.Checked then begin
    mainfilter:=GetCheckedFromList(self.MainTypeCB);
    if length(mainfilter)>0 then mainfilter:=' AND (T1.[MAINTYPE] IN '+mainfilter+' OR T1.[MAINTYPE]=0)';
  end;
  if length(mainfilter)>0 then msg:='' else msg:='Вы не выбрали ни одного типа продукции!'+chr(13)+
      'Будут показаны записи с выбранными кодами для всех типов продукцмии.';
  str:='';
  if (WorkCodeCheck.Checked)and(WorkCodeCheck.Enabled) then begin
    for I := 0 to WorkCodeCB.Count - 1 do
      if WorkCodeCB.Checked[i] then begin
      pid:=pointer(WorkCodeCB.Items.Objects[i]);
      str:=str+QuotedStr(pid^)+',';
      end;
    if length(str)>0 then begin
      str:=copy(str,1,length(str)-1);
      mainfilter:=mainfilter+' AND (T1.[CODE] IN ('+str+'))';
    end;
  end;
  if (length(msg)>0)and(length(str)=0) then msg:='Вы не выбрали ни одного типа продукции, и ни одного кода!'+chr(13)+
      'Будут выбраны все коды, общие для всех типов продукцмии.';
  if (length(msg)=0)and(length(str)=0) then  msg:='Вы не выбрали ни одного кода!'+chr(13)+
      'Будут выбраны все коды, общие для  выбранных типов продукцмии.';
  if length(msg)>0 then
    if MessageDlg(msg,mtWarning,[mbOk,mbCancel],0)=mrCancel then Abort;
  ...
  while not RowQuery.Eof do begin
    MainGrid.Cells[1,r]:=RowQuery.FieldByName('ID').AsString+' '+RowQuery.FieldByName('DESCR').AsString;
    if MainGrid.Objects[0,r]<>nil then FreeMem(pointer(MainGrid.Objects[0,r]));
    new(pid);
    pid^:=RowQuery.FieldByName('ID').AsString;
...
function  TRepCodetForMonthsForm.GetProdSQL(dt1,dt2 : TDateTime):string;
var
  str       : string;
  txt       : TStringList;
begin
  ...
  txt.Add('WHERE (T1.[DATETIME] BETWEEN '+QuotedStr(FormatDateTime('yyyymmdd',StartOfTheDay(dt1)))+
      ' AND '+QuotedStr(FormatDateTime('yyyymmdd',EndOfTheDay(dt2)))+')');
  //
  str:=self.GetCheckedFromList(self.MainTypeCB);
  if length(str)>0 then txt.Add(' AND MAINTYPE IN '+str);
  //фабрика
  case FactoryCB.ItemIndex of
...
function  TRepCodetForMonthsForm.GetReportFilterCaption : string;
var
  flt,str : string;
  i       : integer;
begin
...
function  TRepCodetForMonthsForm.GetRecordFilterCaption : string;
var
  flt,str : string;
  i       : integer;
  pid     : ^integer;
...
procedure TRepCodetForMonthsForm.ClearClientBtnClick(Sender: TObject);
begin
...
procedure TRepCodetForMonthsForm.CloseBtnClick(Sender: TObject);
begin
  self.Close;
end;

procedure TRepCodetForMonthsForm.CopyBtnClick(Sender: TObject);
var
  c,r:integer;
begin
...
procedure TRepCodetForMonthsForm.ReportGetValue(const VarName: string; var Value: Variant);
var
  str : string;
begin
...
procedure TRepCodetForMonthsForm.SetBtnClick(Sender: TObject);
begin
...
procedure TRepCodetForMonthsForm.GetFirstDateBtnClick(Sender: TObject);
var
  month,year : word;
  txt : string;
begin
...
procedure TRepCodetForMonthsForm.ChangeFilter(Sender: TObject);
begin
  MainGrid.Font.Color:=clGray;
  MainGrid.Repaint;
end;

procedure TRepCodetForMonthsForm.UpdateFilter(Sender: TObject);
var
  i,j : integer;
  pid : ^integer;
  upd : boolean;
begin
  ...
  GroupCheck.Enabled:=(i>0)and(j=1)and(MainTypeCheck.Checked);
  GroupCB.Visible:=(GroupCheck.Checked)and(i>0)and(j=1)and(GroupCheck.Enabled);
...
procedure TRepCodetForMonthsForm.RegionCBChange(Sender: TObject);
begin
...
procedure TRepCodetForMonthsForm.RegionSBClick(Sender: TObject);
begin
  self.UpdateFilter(sender);
  self.RegionCBChange(sender);
end;

function  TRepCodetForMonthsForm.UpdateGroupCB(mt : integer; update : boolean):integer;
var
  i : integer;
  pid : ^integer;
  Table : TADOTable;
...
function  TRepCodetForMonthsForm.UpdateModelCB(mt : integer; update : boolean):integer;
var
  i : integer;
  pid : ^integer;
...
function  TRepCodetForMonthsForm.UpdateSenterCB:integer;
var
  i : integer;
  pid : ^integer;
  Query : TADOQuery;
...
function  TRepCodetForMonthsForm.UpdateCodeCB(mt : integer; update : boolean):integer;
var
  i : integer;
  pid : ^string;
  Query : TADOQuery;
...
procedure TRepCodetForMonthsForm.CheckAllBtnClick(Sender: TObject);
var
  box : TCheckListBox;
  i   : integer;
begin
...
procedure TRepCodetForMonthsForm.FormClose(Sender: TObject;
...
procedure TRepCodetForMonthsForm.FormCreate(Sender: TObject);
begin
...
procedure TRepCodetForMonthsForm.FormResize(Sender: TObject);
begin
  SB.VertScrollBar.Position:=0;
end;

procedure TRepCodetForMonthsForm.FormShow(Sender: TObject);
var
  pid   : ^integer;
  Table : TADOTable;
begin
